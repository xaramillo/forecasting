...
model_dir = os.path.join(f.MODEL_DIR, "vanilla_final")

test_data = pd.read_csv(os.path.join(f.DATA_DIR, "sample_test_data.csv"), index_col=0)

config = load_config(master_config_name='master_config.yaml', model_config_name='vanilla_model_config.yaml')

model = VanillaForecastModel(**config)
...
for i in range(len(test_data)):
    ...
    visualise_prediction(history, true_forecast, prediction, save_name="vanilla_forecast_{}.png".format(i))
